...
def build_admin_summary_xlsx_bytes(selected_date: date) -> bytes:
    """
    Builds an Excel (.xlsx) version of the printable game summary.
    """
    games = get_admin_print_rows_for_date(selected_date)
    ...
    for g in games:
        ...
        r1 = g["slots"][1]["name"] or ""
        r2 = g["slots"][2]["name"] or ""
...
with tabs[0]:
    ...
    with c1:
        if st.button("Build PDF", key="build_pdf_btn"):
            try:
                st.session_state["summary_pdf"] = build_admin_summary_pdf_bytes(selected_date)
                st.success("PDF built.")
            except Exception as e:
                st.error(f"PDF failed: {e}")

    with c2:
        if st.button("Build Excel (.xlsx)", key="build_xlsx_btn"):
            try:
                st.session_state["summary_xlsx"] = build_admin_summary_xlsx_bytes(selected_date)
                st.success("Excel file built.")
            except Exception as e:
                st.error(f"Excel failed: {e}")

    with c3:
        pdf_bytes = st.session_state.get("summary_pdf")
        if pdf_bytes:
            st.download_button(
                label="Download PDF",
                data=pdf_bytes,
        ...
        xlsx_bytes = st.session_state.get("summary_xlsx")
        if xlsx_bytes:
            st.download_button(
        ...
        with st.container(border=True):
            ...
            for idx, a in enumerate(assigns):
                with cols[idx]:
                    ...
                    st.caption(
                        f"assignment_id={a['id']} | status={status} | updated_at={a['updated_at']}"
                    )

                    current_ref_label = None
                    if a["referee_id"] and a["ref_name"] and a["ref_email"]:
                    ...
                    default_index = 0
                    if current_ref_label in ref_options:
                    ...
                    if pick != "— Select referee —":
                        ...
                        if status not in ("ACCEPTED", "ASSIGNED"):
